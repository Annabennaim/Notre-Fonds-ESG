...
st.write("""
Cette première analyse du fonds nous permet de réaliser une première analyse.
...
if not secteurs_df.empty:
    ...
    st.write(""" Le secteur de l’énergie, à l’inverse, est structurellement désavantagé dans les notations ESG du fait de sa dépendance historique aux énergies fossiles.
            Même si de nombreux acteurs commencent à investir dans les renouvelables, le cœur de leur activité repose encore largement sur l’extraction, la transformation et la distribution de pétrole, de gaz et de charbon — activités fortement émettrices de gaz à effet de serre.
    ...
    def analyse_secteur(titre, secteur_cible, phrase_personnalisee):
        ...
        if not entreprises_df.empty:
            ...
            st.write("A noter que le score ESG total n'est pas la moyenne des trois scores. Cela s'explique par la pondération différente des facteurs et l'intégration d'autres éléments tels que le niveau de controverse qui peuvent augmenter ou diminuer le score total.​")
    ...
    analyse_secteur(
        ...
        "L'entreprise ayant la meilleure performance extrafinancière est le groupe CBRE avec un score ESG de 8,96. "
        ...
        "soit une meilleure performance que la 'meilleure' entreprise du secteur de l'Energie."
    )
   ...
   # Analyse du secteur le moins vertueux
    analyse_secteur(
        ...
        "L'entreprise ayant la meilleure performance extrafinancière est Baker Hughes avec un score ESG de 19,12. "
...
"Dans cette dernière partie, nous utilisons nos observations précédentes afin de construire un portefeuillle plus performant vis à vis des critères ESG. "


"Si le secteur Energie est choisi, nous laissons la possibilité d'exclure certaines entreprises. Nous avons fais la meme chose pour le seceteur des matériaux qui regroupe plus d'entreprises et qui est le deuxième secteur de notre fonds ayant le pire score ESG "
